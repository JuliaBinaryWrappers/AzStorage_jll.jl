...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import AzStorage_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("AzStorage_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "AzStorage_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"AzStorage", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "AzStorage_jll", "src", "AzStorage_jll.jl"))
        @info("AzStorage_ll dev'ed out to /depot/dev/AzStorage_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("00c928b4-b5f3-54d8-b38d-afd4635c4ad2"))
platforms = [Pkg.Artifacts.unpack_platform(e, "AzStorage", artifacts_toml) for e in artifacts["AzStorage"]]
...
if best_platform === nothing
    @debug("Unable to load AzStorage; unsupported platform $(triplet(platform_key_abi()))")
else
